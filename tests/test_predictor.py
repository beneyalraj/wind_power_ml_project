"""
tests/test_predictor.py — Unit tests for feature engineering logic in predictor.py.
...
class TestBuildFeatures:
    ...
    def test_wind_direction_sin_is_correct(self):
        ...
        result = build_features([BASE_RECORD], FEATURE_NAMES)
        idx = FEATURE_NAMES.index("wind_direction_sin")
        assert math.isclose(result[0][idx], math.sin(math.radians(90.0)), rel_tol=1e-6)

    def test_wind_direction_cos_is_correct(self):
        """90 degrees → cos(90°) ≈ 0.0"""
        result = build_features([BASE_RECORD], FEATURE_NAMES)
        idx = FEATURE_NAMES.index("wind_direction_cos")
        assert math.isclose(result[0][idx], math.cos(math.radians(90.0)), abs_tol=1e-6)

    def test_wind_speed_squared_is_correct(self):
        """10.0 ** 2 = 100.0"""
        result = build_features([BASE_RECORD], FEATURE_NAMES)
        idx = FEATURE_NAMES.index("wind_speed_squared")
        assert result[0][idx] == pytest.approx(100.0)

    def test_wind_speed_cubed_is_correct(self):
        """10.0 ** 3 = 1000.0"""
        result = build_features([BASE_RECORD], FEATURE_NAMES)
        idx = FEATURE_NAMES.index("wind_speed_cubed")
        assert result[0][idx] == pytest.approx(1000.0)

    def test_wake_adjusted_wind_is_correct(self):
        """wake = wind_speed * (1 - turbulence_intensity) = 10.0 * 0.9 = 9.0"""
        result = build_features([BASE_RECORD], FEATURE_NAMES)
        idx = FEATURE_NAMES.index("wake_adjusted_wind")
        assert result[0][idx] == pytest.approx(9.0)

    def test_feature_order_matches_feature_names(self):
        """
        Features must be in the exact order model was trained on.
        A reordering bug would produce silently wrong predictions.
        """
        result = build_features([BASE_RECORD], FEATURE_NAMES)
        row = result[0]
        assert row[FEATURE_NAMES.index("wind_speed")] == BASE_RECORD["wind_speed"]
        assert row[FEATURE_NAMES.index("num_turbines")] == BASE_RECORD["num_turbines"]
    ...
    def test_zero_wind_speed_produces_zero_derived_features(self):
        ...
        assert result[0][FEATURE_NAMES.index("wind_speed_squared")] == 0.0
        assert result[0][FEATURE_NAMES.index("wind_speed_cubed")]   == 0.0
        assert result[0][FEATURE_NAMES.index("wake_adjusted_wind")] == 0.0

    def test_north_wind_direction_sin_cos_symmetry(self):
        """0 degrees (North): sin=0, cos=1"""
        record = {**BASE_RECORD, "wind_direction": 0.0}
        result = build_features([record], FEATURE_NAMES)
        assert math.isclose(result[0][FEATURE_NAMES.index("wind_direction_sin")], 0.0, abs_tol=1e-6)
        assert math.isclose(result[0][FEATURE_NAMES.index("wind_direction_cos")], 1.0, rel_tol=1e-6)